  ...
  function handleDisconnect() {
      ...
      connection.connect(err => {
          if (err) {
              ...
              setTimeout(handleDisconnect, 2000); // Retry connection after 2 sec
          } else {
...
//View Sizes
  ...
  app.post('/deleteitem', checkAuthenticated,(req,res) => {
    ...
    // for(j=0;j<transaction_id.length;j++){
    //   let query = mysqlConnection.query(sql,[deleteid[j]], (err, rows, fields)=>{
    //     if(!err)
    //     {
    //     console.log('Successfully deleted corresponding values from orderdb')
    //     res.redirect('/orders')
